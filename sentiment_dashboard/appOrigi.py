...
color_map = {
    ...
    'NEGATIVE': '#E74C3C',  # Strong red
    'NEUTRAL':  '#95A5A6'   # Soft gray
}

# --- Helper Function for Neutral Detection ---
def classify_sentiment(label, score, threshold=0.6):
    """Classify sentiment and add NEUTRAL if below threshold."""
...
EMOJI_MAP = {
    ...
    "love": "❤️", "surprise": "😲", "neutral": "😐", "disgust": "🤢",
    "shame": "😳", "guilt": "😔", "confusion": "😕", "admiration": "😍",
    "excitement": "🤩", "pride": "😌", "relief": "😌", "embarrassment": "😳",
    "boredom": "😒", "curiosity": "🤔", "nervousness": "😰", "anticipation": "🤔",
    "gratitude": "🙏", "hope": "🌟", "disappointment": "😞", "frustration": "😤",
...
def run_sentiment_emotion_analysis(text, model_name):
    ...
    positive_words = ['love', 'great', 'amazing', 'wonderful', 'fantastic', 'excellent', 'good', 'happy', 'joy']
    negative_words = ['hate', 'terrible', 'awful', 'bad', 'horrible', 'sad', 'angry', 'frustrated', 'disappointed']
    ...
    if pos_count > neg_count:
        ...
        emotion = np.random.choice(['sadness', 'anger', 'frustration', 'disappointment'])
    else:
        sentiment = "neutral"
        ...
        if quick_text and quick_text.strip():
            with st.spinner("Analyzing..."):
                ...
                st.session_state.history.append({
                    "text": quick_text[:50] + "..." if len(quick_text) > 50 else quick_text,
...
# ─────────────────────────────────────────────────────────────────────────────
st.markdown('<div class="main-header">🤖 Sentiment & Emotion Analyzer</div>', unsafe_allow_html=True)

tab_home, tab_single, tab_batch, tab_viz, tab_history, tab_insights = st.tabs([
    "🏠 Home", "🔍 Single Analysis", "📊 Batch Results", "📈 Visualizations", "📋 History", "💡 Insights"
])
...
with tab_home:
    ...
    with col2:
        ...
        if st.session_state.history:
            st.success(f"🎊 Great! You've already analyzed {len(st.session_state.history)} texts. Check out the other tabs for detailed insights!")
    ...
    if quick_text and quick_text.strip():
        ...
        with col2:
            ...
            if e_score >= confidence:
                emoji_char = st.session_state.emoji_map.get(emotion, "❓") if show_emojis else ""
                st.markdown(f"""
                ...
                st.info(f"🔍 Emotion confidence ({e_score:.2f}) is below your threshold ({confidence:.2f})")
        ...
        st.info("👈 Enter some text in the sidebar and click 'Analyze Now' to see detailed analysis here!")
...
with tab_batch:
    ...
    if uploaded_file:
        try:
            ...
            if not is_valid:
                st.error(f"❌ {message}")
                st.info("💡 **Tip**: Your file should have a 'text' column containing the texts to analyze.")
                return
            ...
            col1, col2 = st.columns(2)
            with col1:
                max_rows = st.number_input("Max rows to process", 1, len(df), min(len(df), 1000))
            with col2:
            ...
            if process_button or 'batch_results' in st.session_state:
                ...
                with st.spinner("🔄 Analyzing texts... This may take a moment."):
                    ...
                    for i, (_, row) in enumerate(df_subset.iterrows()):
                        ...
                        if text:
                            res = run_sentiment_emotion_analysis(text, st.session_state.model_name)
                            results.append(res)
                        else:
                            results.append(("neutral", 0.5, "neutral", 0.5))
                ...
                col4.metric("Avg Score", f"{out_df['sentiment_score'].mean():.2f}")
                ...
                with col1:
                    ...
                    st.download_button(
                        "📊 Download CSV",
                        data=csv_data,
                        file_name=f"sentiment_analysis_{uploaded_file.name.split('.')[0]}.csv",
                        mime="text/csv"
                    )
                ...
                with col2:
                    ...
                    Total texts analyzed: {len(out_df)}
                    
                    Sentiment Distribution:
                    - Positive: {sentiment_counts.get('positive', 0)} ({sentiment_counts.get('positive', 0)/len(out_df)*100:.1f}%)
                    - Negative: {sentiment_counts.get('negative', 0)} ({sentiment_counts.get('negative', 0)/len(out_df)*100:.1f}%)
                    - Neutral: {sentiment_counts.get('neutral', 0)} ({sentiment_counts.get('neutral', 0)/len(out_df)*100:.1f}%)
                    
                    Average Scores:
                    - Sentiment: {out_df['sentiment_score'].mean():.3f}
                    - Emotion: {out_df['emotion_score'].mean():.3f}
                    """
                    
                    st.download_button(
                        "📋 Download Report",
                        data=summary,
                        file_name=f"analysis_report_{uploaded_file.name.split('.')[0]}.txt",
                        mime="text/plain"
                    )
        
        except Exception as e:
            st.error(f"❌ Error processing file: {str(e)}")
            st.info("💡 Please ensure your file is a valid CSV or XLSX with a 'text' column.")
    else:
    ...
    if viz_data is not None and not viz_data.empty:
        ...
        with viz_tab3:
            ...
            if len(viz_data) > 5:
                ...
                heatmap_data = viz_data.groupby(['sentiment', 'emotion']).size().reset_index(name='count')
                pivot_data = heatmap_data.pivot(index='sentiment', columns='emotion', values='count').fillna(0)
        ...
        with viz_tab4:
            ...
            col4.metric("Most Common", viz_data['emotion'].mode()[0] if not viz_data['emotion'].mode().empty else "N/A")
            ...
            with col1:
                ...
                for emotion, count in top_emotions.items():
                    ...
                    st.write(f"{emoji} **{emotion.title()}**: {count} ({percentage:.1f}%)")
            ...
            with col2:
                ...
                for sentiment, count in sent_breakdown.items():
                    ...
                    st.write(f"{color} **{sentiment.title()}**: {count} ({percentage:.1f}%)")
...
with tab_history:
    ...
    if st.session_state.history:
        st.success(f"📊 You have {len(st.session_state.history)} analyses in your history")
        ...
        with col3:
            sort_by = st.selectbox("Sort by", ["Recent first", "Oldest first", "Highest score", "Lowest score"])
        ...
        if sort_by == "Recent first":
            ...
            hist_df = hist_df.sort_values('sent_score', ascending=False)
        elif sort_by == "Lowest score":
            hist_df = hist_df.sort_values('sent_score', ascending=True)
        ...
        if not hist_df.empty:
            ...
            # Add emoji column
            if show_emojis:
                display_df['emoji'] = display_df['emotion'].map(st.session_state.emoji_map)
            
            # Format scores
            display_df['sent_score'] = display_df['sent_score'].round(2)
            display_df['emo_score'] = display_df['emo_score'].round(2)
            ...
            col2.metric("Avg Sentiment", f"{hist_df['sent_score'].mean():.2f}")
            col3.metric("Most Common Emotion", hist_df['emotion'].mode()[0] if not hist_df['emotion'].mode().empty else "N/A")
            col4.metric("Positive Ratio", f"{(hist_df['sentiment'] == 'positive').mean():.1%}")
        ...
        with col2:
            if st.button("📋 Export Summary"):
                ...
                Average Scores:
                Sentiment: {summary_data['sent_score'].mean():.3f}
                Emotion: {summary_data['emo_score'].mean():.3f}
                """
                st.download_button(
                    "Download Summary",
                    data=summary,
...
with tab_insights:
    ...
    if insights_data is not None and len(insights_data) > 0:
        st.info(f"🎯 Generating insights from {data_source} ({len(insights_data)} items)")
        ...
        most_common_emotion = insights_data['emotion'].mode()[0] if not insights_data['emotion'].mode().empty else "N/A"
        ...
        if avg_sentiment > 0.7:
            insights.append(("🌟", "Highly Positive", f"Your texts show overwhelmingly positive sentiment (avg: {avg_sentiment:.2f})"))
        elif avg_sentiment < 0.3:
            insights.append(("⚠️", "Concerning Negativity", f"Many texts show negative sentiment (avg: {avg_sentiment:.2f})"))
        else:
            insights.append(("⚖️", "Balanced Sentiment", f"Your texts show balanced sentiment (avg: {avg_sentiment:.2f})"))
        
        if sentiment_std < 0.2:
            insights.append(("📏", "Consistent Tone", f"Very consistent sentiment across texts (std: {sentiment_std:.2f})"))
        elif sentiment_std > 0.4:
            insights.append(("🎢", "Variable Emotions", f"High emotional variability in your texts (std: {sentiment_std:.2f})"))
        
        if positive_ratio > 0.8:
            insights.append(("😊", "Optimistic Content", f"{positive_ratio:.1%} of your content is positive"))
        elif positive_ratio < 0.2:
            insights.append(("😟", "Pessimistic Trend", f"Only {positive_ratio:.1%} of your content is positive"))
        ...
        with col1:
            ...
            st.metric("Emotion Diversity", f"{unique_emotions}/{total_possible}", f"{emotion_diversity:.1%}")
            
            # Sentiment consistency
            sentiment_consistency = 1 - sentiment_std
            st.metric("Sentiment Consistency", f"{sentiment_consistency:.2f}", "Higher is more consistent")
            ...
            if len(insights_data) > 1:
                ...
                st.caption(f"Score: {insights_data.loc[most_positive_idx, 'sentiment_score']:.2f}")
        ...
        with col2:
            ...
            if avg_sentiment < 0.4:
                recommendations.append("Consider incorporating more positive language in your content")
            
            if sentiment_std > 0.4:
                recommendations.append("Your content shows high emotional variability - consider maintaining consistent tone")
            
            if unique_emotions < 5:
                recommendations.append("Try expressing a wider range of emotions to engage your audience better")
            
            if positive_ratio < 0.3:
                recommendations.append("Balance negative content with positive elements for better engagement")
            
            if not recommendations:
                recommendations.append("Your content shows good emotional balance and consistency!")
        ...
        st.info("💡 Based on your analysis patterns, future texts are likely to be " + 
...
def plot_sentiment_over_time(df):
    ...
    time_counts = df.groupby(['date', 'sentiment']).size().unstack(fill_value=0)

    fig, ax = plt.subplots(figsize=(8, 4))
    for sentiment in time_counts.columns:
        ax.plot(time_counts.index, time_counts[sentiment],
                label=sentiment, color=color_map[sentiment], marker='o', linewidth=2)
        for x, y in zip(time_counts.index, time_counts[sentiment]):
            ax.text(x, y + 0.1, str(y), ha='center', fontsize=8)
...
if uploaded_file is not None:
    ...
    if 'text' not in df.columns:
        ...
        df['sentiment'] = [classify_sentiment(r['label'], r['score']) for r in results]
        ...
        col_kpi1.metric("📄 Total Texts", total_count)
        col_kpi2.metric("✅ Positive", f"{pos_count} ({pos_count/total_count:.0%})", delta=f"{pos_count - neg_count}")
        col_kpi3.metric("😡 Negative", f"{neg_count} ({neg_count/total_count:.0%})", delta=f"{neg_count - pos_count}")
        col_kpi4.metric("😐 Neutral", f"{neu_count} ({neu_count/total_count:.0%})")
...
# --- Manual Input ---
st.subheader("🔍 Test Single Sentence")
user_input = st.text_area("Enter text to analyze:")

if st.button("Analyze Sentiment"):
    if user_input.strip():
        result = sentiment_analyzer(user_input)[0]
        label = classify_sentiment(result['label'], result['score'])
